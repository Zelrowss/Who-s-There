...
float2 DecodeFloat (float enc)
{
    float2 result ;
    result.y = enc % 2048;
    result.x = floor(enc * rcp(2048));
    return result * rcp(2048 - 1);
}
...
//  Branch Wind
    ...
    //      Just a simple triangle wave in 3D
            ...
            perVertexRandom += SmoothTriangleWave(trigWave.x) * 2.0 - 1.0;
    ...
    //  We skip mainWindStrength here to prevent branchBending from going nuts. Instead we factor in _LuxInstantTurbulence
        float branchBending = (SmoothTriangleWave(Time * _BranchWindSpeed + phase) * 2.0 - 1.0); // * mainWindStrength; 
    //  Get mask
